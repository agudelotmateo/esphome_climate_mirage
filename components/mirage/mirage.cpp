...
void MirageClimate::transmit_state() {
  this->last_transmit_time_ =
      millis();  // setting the time of the last transmission.
  ...
  auto temp = (uint8_t)roundf(
      clamp(this->target_temperature, MIRAGE_TEMP_MIN, MIRAGE_TEMP_MAX));
  remote_state[1] += temp;
  ...
  ESP_LOGI(TAG,
           "Sending: %02X %02X %02X %02X   %02X %02X %02X %02X   %02X %02X "
           "%02X %02X   %02X %02X",
  ...
  esphome::remote_base::MirageProtocol obj =
      esphome::remote_base::MirageProtocol();
  obj.encode(data, in);
...
bool MirageClimate::on_receive(remote_base::RemoteReceiveData data) {
  ...
  esphome::remote_base::MirageProtocol obj =
      esphome::remote_base::MirageProtocol();
  optional<esphome::remote_base::MirageData> optional_data_decoded =
      obj.decode(data);
  ...
  const esphome::remote_base::MirageData &data_decoded =
